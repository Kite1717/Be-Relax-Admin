...
import { CreateSleepCategory } from "./CreateSleepCategory"
...
export function Sleep() {
    ...
    useEffect(() => {
        let temp = [];
        firestore.collection("sleepCategories").get().then(querySnapshots => {
    ...
    const onSubChange = e => {
        ...
        uploadControl();
        let temp = [];
        firestore.collection("sleepAlbums").get().then(querySnapshots => {
            querySnapshots.docs.map(doc => {
                if (doc.data().blob) {
                    console.log(doc.data().blob)
                } else {
    ...
    const handleAlbumDelete = (albumOption) => {
        ...
        firestore.collection("sleepAlbums").where("albumId", "==", albumOption).get().then(querySnapshots => {
        ...
        folder1 = "sleep" + "/" + subOption.id + "/" + albumOption + "/" + "images"
        folder2 = "sleep" + "/" + subOption.id + "/" + albumOption + "/" + "sounds"
        folder3 = "sleep" + "/" + subOption.id + "/" + albumOption
        folder4 = "sleep" + "/" + subOption.id
    ...
    const handleSubDelete = () => {
        let deleteRef1 = storage.ref(`sleep/${subOption.id}`)

        storage.ref(`sleep/${subOption.id}`).listAll().then(list => {
            setIsLoading(true);
            list.prefixes.map(e => {
            ...
            deleteRef1.listAll().then(list => {
                list.items.map(e => {
                    ...
                    ref.delete().then(obj => {
                        let temp = [];
                        storage.ref("sleep").listAll().then(list => {
                            list.prefixes.map(e => {
                                temp.push(e.name)
                            })
                        }).then(obj => {
                            firestore.collection("sleepCategories").where("id", "==", subOption.id).get().then(querySnapshots => {
                                querySnapshots.docs[0].ref.delete();
                            }).then(res => {
    ...
    return (
        <>
            <Card>
                <CardHeader style={{ justifyContent: "center" }} title="Uyku">
                </CardHeader>
            </Card>
            <br />
                                        ...
                                        if (!image.deleted) {
                                            return (
                                                <div className="col-md-5-cols" >
                                                    ...
                                                    <br />
                                                    <Button key={index} onClick={() => onImageDeleteClicked(index)} >Sil</Button>
                                                </div>
                                            )
                                        }
                                        ...
                                        if (!sound.deleted) {
                                            return (
                                                <div>
                                                    ...
                                                    <br />
                                                    <Button key={index} onClick={() => onSoundDeleteClicked(index)} >Sil</Button>
                                                </div>
                                            )
                                        }
            ...
            {
                showUpload &&
                <UploadModal  setIsUploadOk={setIsUploadOk} setIsImageFive={setIsImageFive} show={showUpload} onClose={handleShowUpload} setSounds={setSounds} sounds={sounds} images={images} setImages={setImages} subOption={subOption} setAlbums={setAlbums} />
            }


            {
                showSubCreate &&
                <CreateSleepCategory show={showSubCreate} onClose={onSubCreate} setSubs={setSubs} />
            }
            {
                showSoundUpload &&
