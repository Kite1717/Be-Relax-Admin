...
export function Music() {
    ...
    const handleAlbumDelete = (albumOption) => {
        ...
        firestore.collection("musicAlbums").where("albumId", "==", albumOption).get().then(querySnapshots => {
        ...
        folder1 = "music" + "/" + subOption.id + "/" + albumOption + "/" + "images"
        folder2 = "music" + "/" + subOption.id + "/" + albumOption + "/" + "sounds"
        folder3 = "music" + "/" + subOption.id + "/" + albumOption
        folder4 = "music" + "/" + subOption.id
    ...
    const handleSubDelete = () => {
        let deleteRef1 = storage.ref(`music/${subOption.id}`)

        storage.ref(`music/${subOption.id}`).listAll().then(list => {
            setIsLoading(true);
            list.prefixes.map(e => {
            ...
            deleteRef1.listAll().then(list => {
                list.items.map(e => {
                    ...
                    ref.delete().then(obj => {
                        let temp = [];
                        storage.ref("music").listAll().then(list => {
                            list.prefixes.map(e => {
                                temp.push(e.name)
                            })
                        }).then(obj => {
                            firestore.collection("musicCategories").where("id", "==", subOption.id).get().then(querySnapshots => {
                                querySnapshots.docs[0].ref.delete();
                            }).then(res => {
    ...
    return (
        <>
            <Card>
                <CardHeader style={{ justifyContent: "center" }} title="Müzik">
                </CardHeader>
            </Card>
            <br />
            ...
            {
                showSubCreate &&
                <CreateMusicCategory show={showSubCreate} onClose={onSubCreate} setSubs={setSubs} />
            }
            {
                showSoundUpload &&
