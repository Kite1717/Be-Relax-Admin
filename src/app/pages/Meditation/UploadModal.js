...
export function UploadModal({ setIsImageFive, show, onClose, setSounds, sounds, images, setImages, subOption, setAlbums, setIsUploadOk }) {
    ...
    const firestoreUpload = () => {
        ...
        firestore.collection("meditationAlbums").add((firestoreObjTemp)).then(res => {
    ...
    const handleUpload = async () => {
        ...
        if (albumName !== "" && authorName !== "" && description !== "" && duration !== 0 && teller !== "") {
            ...
            temp.map((obj,index) => {
                ...
                var folder = "";
                folder = "meditation" + "/" + subOption.id + "/" + albumId + "/" + fileType;
                            ...
                            if (authorUrl !== "" && bannerUrl !== "" && headerUrl !== "" && tellerUrl !== "" && modalUrl !== "" && musics.indexOf(-1) === -1) {
                               Promise.all(promises)
                                    .then(() => {
                                        ...
                                        firestore.collection("meditationAlbums").get().then(queryParams => {
                                            let temp = [];
